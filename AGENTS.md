...
| Schedule setup | `model/schedule/mod.rs` (the loader fleet and its rules), `model/schedule/calendar.rs` (loader defaults and sparse period overrides), `model/schedule/production.rs` (per-period tonnes read back off a calculated run), `app/commands/schedule.rs` (undoable edits), `ui/elements/schedule_setup.rs`, `ui/elements/schedule_calendar.rs`, and `ui/dialogs/schedule.rs`; the plan lives on `Document` and is read by UI through `UiProjectView::schedule`, not the scene composite. |
