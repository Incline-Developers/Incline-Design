...
impl<R: ReadAt> parquet::file::reader::ChunkReader for SubFile<R> {
    ...
    fn get_bytes(&self, start: u64, length: usize) -> parquet::errors::Result<bytes::Bytes> {
        // Must return exactly `length` bytes: callers parse the buffer without re-checking
        // its bounds, so any extra is read as if it were part of the requested range.
        let mut buf = vec![0; length];
        self.get_read(start)?.read_exact(&mut buf)?;
        Ok(buf.into())
    }
}
