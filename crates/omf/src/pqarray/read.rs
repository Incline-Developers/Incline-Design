...
fn check<P: PqArrayType>(
    ...
    if !ty.is_primitive()
        ...
        || !P::check_logical_type(ty.get_basic_info().logical_type_ref().cloned())
    {
    ...
    Ok(Info {
        column_index,
        logical_type: ty.get_basic_info().logical_type_ref().cloned(),
...
impl<R: ChunkReader + 'static> PqArrayReader<R> {
    ...
    /// Read `N` required columns of one type into a single vector of rows.
    ///
    /// This is the bulk counterpart of [`Self::iter_multi_column`]. Row groups
    /// carry independent data and their row counts are in the file metadata,
    /// so each one decodes on its own thread straight into its slice of the
    /// output. Within a group every column is drained in `read_records`
    /// blocks, where the iterator hands back one `Option<Result<_>>` per
    /// value - plumbing that costs more than the Parquet decoding it wraps
    /// once an array runs to tens of millions of points.
    pub fn read_multi_column<P, const N: usize>(
        &self,
        names: [&str; N],
    ) -> Result<Vec<[P; N]>, Error>
    where
        P: PqArrayType + Copy + Send + Sync,
    {
        let metadata = self.file_reader.metadata();
        let infos: [Info; N] = names
            .iter()
            .map(|name| check::<P>(metadata, ColumnPath::new(vec![(*name).to_owned()]), false))
            .collect::<Result<Vec<_>, _>>()?
            .try_into()
            .expect("correct length");
        ...
        let counts: Vec<usize> = (0..metadata.num_row_groups())
            ...
            .collect();
        let mut rows = vec![[P::default(); N]; counts.iter().sum()];
        ...
        groups
            .into_par_iter()
            .enumerate()
            .try_for_each(|(group, slice)| self.read_row_group(group, &infos, slice))?;
        Ok(rows)
    }

    fn read_row_group<P, const N: usize>(
        &self,
        group: usize,
        infos: &[Info; N],
        rows: &mut [[P; N]],
        ...
        P: PqArrayType + Copy,
    {
        ...
        for (lane, info) in infos.iter().enumerate() {
            ...
            while filled < rows.len() {
                ...
                for (row, value) in rows[filled..].iter_mut().zip(values.drain(..)) {
