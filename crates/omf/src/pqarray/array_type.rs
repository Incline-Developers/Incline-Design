use chrono::prelude::*;
use parquet::{
    basic::{Encoding, IntType, LogicalType, TimeUnit, TimestampType, Type as PhysicalType},
...
impl PqArrayType for DateTime<Utc> {
    ...
    fn check_logical_type(file_type: Option<LogicalType>) -> bool {
        matches!(file_type, Some(LogicalType::Timestamp(_)))
    }
    ...
    fn from_parquet(
        ...
        match logical_type {
            Some(LogicalType::Timestamp(TimestampType { unit, .. })) => match unit {
                TimeUnit::MILLIS => i64_milli_to_date_time(value),
                TimeUnit::MICROS => i64_to_date_time(value),
                TimeUnit::NANOS => i64_nano_to_date_time(value),
            },
...
mod tests {
    ...
    const DATE_TIME_MILLI: Option<LogicalType> = Some(LogicalType::Timestamp(TimestampType {
        is_adjusted_to_u_t_c: true,
        unit: TimeUnit::MILLIS,
    }));
    const DATE_TIME_MICRO: Option<LogicalType> = Some(LogicalType::Timestamp(TimestampType {
        is_adjusted_to_u_t_c: true,
        unit: TimeUnit::MICROS,
    }));
    const DATE_TIME_NANO: Option<LogicalType> = Some(LogicalType::Timestamp(TimestampType {
        is_adjusted_to_u_t_c: true,
        unit: TimeUnit::NANOS,
    }));
